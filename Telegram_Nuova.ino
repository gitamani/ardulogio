...
char password[] = "passwordntwork"; // your network key
char ser; //stringa del messaggio ricevuto dalla seriale
String s; //stringa inviata a Telegram
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i=0; i<numNewMessages; i++) {
    ...
    if (from_name == "") from_name = "Giuseppe";
...
void loop() {
  ...
  if (millis() > Bot_lasttime + Bot_mtbs)  {
    ...
    if (stringa!="") {
      sendMessageToAllSubscribedUsers(stringa); 
    }
  }
